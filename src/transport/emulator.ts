// ADBPD — EmulatorTransport (hybrid per-emulator stock-adb backend).
//
// For each detected emulator, we spawn ONE private stock-adb server on a
// dedicated port (default base 5040) with `--one-device <serial>`. This
// gives the emulator full isolation from every other tool on the host and
// from every other emulator.
//
// ADBPD's smart socket routes `host:transport:<serial>` to the private
// backend port. Clients see a single 5037 endpoint; the backend stays
// invisible.
//
...
const log = getLogger('emulator-transport');

const HOST = '127.0.0.1';
const CONNECT_TIMEOUT_MS = 10_000;
const PING_TIMEOUT_MS = 3_000;
const READY_POLL_INTERVAL_MS = 250;
const READY_TOTAL_TIMEOUT_MS = 15_000;
...
export class EmulatorTransport implements DeviceTransport {
  ...
  private readonly opts: EmulatorTransportOptions;
  ...
  async connect(): Promise<void> {
    ...
    log.info({ serial: this.serial, backendPort: this.port }, 'starting backend adb server');

    const env = {
      ANDROID_ADB_SERVER_PORT: String(this.port),
    };
    const args = [
      '--one-device',
      this.serial,
    ...
    this.backend = (this.opts.spawnAdbServer ?? defaultSpawn)(env, [this.opts.adbBinaryPath, ...args]);
    ...
    // Wait until the backend server is accepting connections AND reports the
    // device as online via host:devices.
    ...
    while (Date.now() < deadline) {
      try {
        ...
        if (list.includes(this.serial)) {
          this.setState('online');
          log.info({ serial: this.serial }, 'backend ready, device online');
          return;
        }
      } catch (err) {
    ...
    throw new Error(`EmulatorTransport(${this.serial}): backend not ready within ${READY_TOTAL_TIMEOUT_MS}ms`);
  }

  async disconnect(): Promise<void> {
    if (this.backend !== undefined) {
      log.info({ serial: this.serial, pid: this.backend.pid }, 'stopping backend adb');
      // Graceful kill-server through the backend, then SIGKILL fallback.
      try {
  ...
  async shell(command: string): Promise<ShellResult> {
    // Open a single ADB connection: host:transport:<serial> then shell:cmd.
    // The backend handles the actual device IO; we just read stdout until EOF.
  ...
  async push(_localPath: string, _remotePath: string): Promise<void> {
    throw new Error('EmulatorTransport.push not implemented yet (P2 scope)');
  }
  async pull(_remotePath: string, _localPath: string): Promise<void> {
    throw new Error('EmulatorTransport.pull not implemented yet (P2 scope)');
  }
  async install(_apkPath: string, _opts?: InstallOptions): Promise<void> {
    throw new Error('EmulatorTransport.install not implemented yet (P2 scope)');
  }

  async forward(_local: string, _remote: string): Promise<void> {
    throw new Error('EmulatorTransport.forward not implemented yet (P4 scope)');
  }
  async listForwards(): Promise<Forward[]> {
    return [];
  }
  async removeForward(_local: string): Promise<void> {
    /* noop for P2 */
  }
  ...
  off(event: 'state-change' | 'error', handler: (...args: never[]) => void): void {
    if (event === 'state-change') this.stateHandlers.delete(handler as unknown as StateChangeHandler);
  ...
  openBackend(): Promise<net.Socket> {
    return new Promise<net.Socket>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        sock.destroy();
        reject(new Error(`EmulatorTransport(${this.serial}): backend connect timeout`));
      }, CONNECT_TIMEOUT_MS);
      sock.once('connect', () => {
        clearTimeout(timer);
        resolve(sock);
...
function defaultSpawn(env: Record<string, string>, argv: readonly string[]): SpawnedProcess {
  // Bun spawn: stdout/stderr ignored to avoid blocking on pipes.
...
/**
 * Buffered socket reader — owns the receive buffer for a socket so multiple
 * sequential `readExact` calls can be made without losing bytes.
 *
 * We pause the socket's data event, accumulate, and slice on demand. This
 * avoids `socket.unshift` which has been observed to segfault under Bun.
 */
...
/** Convenience for callers that only need a single read. */
export async function readExact(
  ...
  const reader = new SocketReader(sock);
  return await reader.readExact(n, timeoutMs);
}
