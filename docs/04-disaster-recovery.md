...
- **Port 5037 zombie listener** (**known limitation, in-process fix not possible**). After ADBPD shuts down under NSSM supervision, the kernel can keep the 5037 listen socket bound to the dead pid. Cause: NSSM holds an inherited reference to the bun child's socket handle and doesn't release it until NSSM itself rotates. Even a clean exit (forceful socket `destroy()` in `SmartSocketProxy.stop()`) doesn't help. Symptoms: `netstat -ano | findstr 5037` shows LISTENING on a pid that `Get-Process` says doesn't exist; service restart loops on EADDRINUSE. The smart-socket has an EADDRINUSE retry loop (12 × 5s) that helps with brief zombies but cannot outwait a sustained inherited-handle leak. **In-session escape hatch (verified during the v1.0.0 4h soak + v1.0.1 NSSM experiments):** from an elevated shell, `Stop-Process -Id <nssm-pid> -Force` where the NSSM pid is `(Get-CimInstance Win32_Service -Filter "Name='ADBPD'").ProcessId`. Killing NSSM releases its inherited socket handle; SCM auto-restarts the service into a clean state. **v1.0.1 experimented with NSSM config fixes — none worked:** `AppNoConsole 1` alone did not change the inheritance behavior; removing `AppStdout`/`AppStderr` redirection did not either; `AppDontSpawnConsole` is not actually a valid NSSM parameter (was a misreading of NSSM docs). **The architectural fix is real engineering, not a config tweak.** Two paths for v1.2: (a) FFI to `SetHandleInformation(socket, HANDLE_FLAG_INHERIT, 0)` from the smart-socket code so NSSM cannot inherit even with `bInheritHandles=TRUE`; (b) replace NSSM with a custom service wrapper (or WinSW) that uses `CreateProcess(bInheritHandles=FALSE)`. Path (a) is the smaller change but requires Bun FFI work to reach into Node's `net.Server._handle` to get the underlying SOCKET HANDLE; path (b) is more code but architecturally cleaner.
