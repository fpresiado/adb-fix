...
- **Port 5037 zombie listener.** After a buggy crash-loop, Windows can leave a listen socket bound to a dead pid that even elevated `Stop-Process` can't clear. The cleanest reset is a reboot. (`netsh int ipv4 reset` works too but is heavier.)
