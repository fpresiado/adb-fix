...
| P9 — Soak test | **AMBER** (validation GREEN, 4h production deferred) | 2026-06-01 | 2026-06-01 | 30-min validation soak: 147/148 ops pass (99.3%), 0 unrecovered incidents, all 2 incidents auto_resolved=1. Fault #0 (kill qemu): recovered in 45.8s — over the 30s SLO by ~15s but recovery itself worked end-to-end (watchdog detected, cleanup-sweep fired, EmulatorManager.startAvd relaunched, transport reconnected, shell echo round-tripped). Fault #2 (service-kill via `adb kill-server`) did NOT recover — exposed the NSSM handle-inheritance zombie pattern documented in `docs/04-disaster-recovery.md` (not a code regression, a Windows + NSSM architectural limit). 4h endurance + memory-leak soak deferred to v1.1 along with the NSSM-inheritance fix. |
