...
| P8 — Windows service | **AMBER** (installer ready, owner-reboot pending) | 2026-05-31 | — | `scripts/install-service.ps1` writes the NSSM service (LocalSystem, AutoStart, restart-on-exit, stdout/stderr to logs/, env vars baked). Script is idempotent: removes prior install before installing. Includes post-install /health smoke test. **Reboot verification is owner-side** — Claude cannot reboot the build host. Verification procedure printed at end of installer; commands the owner must run after reboot: `Get-Service ADBPD`, `Invoke-RestMethod http://127.0.0.1:3002/health`, `adb -P 5037 devices` — all must pass BEFORE opening Android Studio. |
